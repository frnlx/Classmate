...
export const prefetch = {
  ...
  classroom: {
    ...
    async categorylist(classid: string) {
      ...
      color.yellow('Fetching Category List of ... and Classroom List... and User Data...')
      ...
      cached.user = data 
      cached.userClassrooms = new Map(data.classes.map(cl => [cl.id, cl]))
      cached.userClassroomsCategories.set(classid, new Map(data.classes.find(c => c.id === classid)?.categories.map(cat => [cat.id, cat])))
